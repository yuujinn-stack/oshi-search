 ...
 */
export default function PersonMultiSelect({ persons, postedPersonNames, selected, onChange }: Props) {
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="border border-gray-200 rounded-lg overflow-y-auto" style={{ maxHeight: 280 }}>
          ...
          {filtered.map((p) => {
            ...
            return (
              <label
                key={p.name}
                className={`flex items-center gap-2 px-3 py-2 text-xs border-b border-gray-50 last:border-0 cursor-pointer transition-colors ${
      ...
      <div>
        <p className="text-xs font-semibold text-gray-500 mb-2">
          選択中（{selected.length}人）— この順番で09:00→15:00→20:00...へ割り当てられます
        </p>
        <div className="border border-gray-200 rounded-lg overflow-y-auto" style={{ maxHeight: 280 }}>
          {selected.length === 0 && (
          ...
          {selected.map((name, i) => (
            <div key={name} className="flex items-center gap-2 px-3 py-2 text-xs border-b border-gray-50 last:border-0">
              ...
              {postedPersonNames.has(name) && (
                <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded shrink-0">投稿済み</span>
              )}
              <button
                type="button"
                onClick={() => moveUp(i)}
