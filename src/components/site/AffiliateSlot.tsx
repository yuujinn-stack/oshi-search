...
function AdWrapper({ children }: { children: ReactNode }) {
  return (
    <div className="affiliate-slot">
...
function renderBranch(creative: ResolvedAffiliateCreative | null, fallback: ReactNode): ReactNode {
  if (creative) return <AdWrapper><CreativeBody creative={creative} /></AdWrapper>;
  return fallback;
}
...
export default async function AffiliateSlot({ vodService, slotKey, fallback = null, className }: AffiliateSlotProps) {
  ...
  if (desktop && mobile && desktop.id === mobile.id) {
    return <div className={className}>{renderBranch(desktop, fallback)}</div>;
  }

  return (
    <div className={className}>
      <div className="hidden md:block">{renderBranch(desktop, fallback)}</div>
      <div className="md:hidden">{renderBranch(mobile, fallback)}</div>
    </div>
  );
}
