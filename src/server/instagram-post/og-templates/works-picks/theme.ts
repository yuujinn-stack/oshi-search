/**
 * 「works-picks」テンプレート（出演作3選、人物写真なし）専用のデザイン設定。
 * works-only・default-personとは独立しており、値を共有しない
 * （将来テンプレートごとに個別調整できるようにするため）。
 * 配色トーンは推しサーチらしさを保つため他テンプレートと揃えている。
 * Instagramの正方形投稿（1080×1080）用。
 */

export const CANVAS = {
  width: 1080,
  height: 1080,
} as const;

export const FONT_FAMILY = 'Noto Sans JP';

export const COLORS = {
  background: '#ffffff',
  accent: '#1fb6e0',
  accentDark: '#0e88ac',
  accentSoft: '#e3f7fd',
  textDark: '#103349',
  white: '#ffffff',
  decorRgb: '31,182,224',
  cardBorder: '#cfeaf5',
  panelBackground: '#e4f6fc',
  panelBorder: '#d3edf7',
  artCardBackground: '#eef8fc',
  artCardBorder: '#bfe8f5',
} as const;

export const BRAND = {
  label: '推しサーチ',
  fontSize: 26,
  letterSpacing: 2,
} as const;

export interface DecorCircleConfig {
  size: number;
  top?: number;
  bottom?: number;
  left?: number;
  right?: number;
  opacity: number;
}
export interface DecorDotConfig {
  size: number;
  top: number;
  left?: number;
  right?: number;
}

// ─── 1枚目: 表紙（「3選」ランキング風の番号バッジで表現、人物写真は使わない） ───────
export const PAGE1 = {
  brandLabelMarginTop: 40,
  title: {
    marginTop: 20,
    fontSize: 48,
    lineHeight: 1.25,
    paddingX: 70,
  },
  buildTitleLines: (personName: string): [string, string] => [`${personName}の出演作`, '3選'],
  /** 「1・2・3」の番号バッジを収める装飾エリア（下部余白が空きすぎないよう十分な高さを確保） */
  artCard: {
    width: 760,
    height: 630,
    marginTop: 40,
    borderRadius: 40,
    borderWidth: 4,
  },
  badge: {
    size: 140,
    fontSize: 56,
  },
  cta: {
    marginTop: 40,
    fontSize: 30,
    text: '出演作をまとめてチェック',
  },
  decorations: {
    circles: [
      { size: 360, left: -130, bottom: -130, opacity: 0.28 },
      { size: 380, right: -140, bottom: -100, opacity: 0.2 },
      { size: 180, right: -60, top: 30, opacity: 0.16 },
    ] as DecorCircleConfig[],
    dots: [] as DecorDotConfig[],
  },
} as const;

// ─── 2枚目: 出演作3件のカード一覧 ─────────────────────────────────────────────
export const PAGE2 = {
  brandLabelMarginTop: 36,
  title: {
    marginTop: 12,
    fontSize: 40,
  },
  buildTitle: (personName: string): string => `${personName}の出演作 3選`,
  panel: {
    marginTop: 26,
    width: 860,
    borderRadius: 44,
    borderWidth: 1,
    paddingY: 26,
    paddingX: 34,
    gap: 18,
    boxShadow: '0 16px 32px rgba(16,51,73,0.08)',
  },
  card: {
    height: 216,
    borderRadius: 26,
    borderWidth: 1.5,
    paddingY: 16,
    paddingX: 22,
    gap: 22,
    boxShadow: '0 6px 14px rgba(16,51,73,0.06)',
    image: {
      width: 110,
      height: 154,
      borderRadius: 14,
    },
    rankBadgeSize: 40,
    titleFontSize: 26,
    titleLineHeight: 1.25,
    vodFontSize: 20,
    vodPaddingY: 6,
    vodPaddingX: 18,
  },
  decorations: {
    circles: [
      { size: 340, left: -130, bottom: -130, opacity: 0.24 },
      { size: 360, right: -140, bottom: -110, opacity: 0.18 },
    ] as DecorCircleConfig[],
    dots: [] as DecorDotConfig[],
  },
} as const;

// ─── 3枚目: CTA専用ページ ────────────────────────────────────────────────────
export const PAGE3 = {
  brandLabelMarginTop: 0,
  title: {
    marginTop: 18,
    fontSize: 48,
    lineHeight: 1.25,
  },
  buildTitleLines: (personName: string): [string, string] => [`${personName}の出演作を`, 'もっとチェック'],
  subtitle: {
    marginTop: 12,
    fontSize: 23,
    lineHeight: 1.4,
    lines: ['推しサーチで出演作・配信先を', 'まとめて確認'] as [string, string],
  },
  /** 1枚目と同じ番号バッジを再掲し、「3選」ブランドの一貫性を出す装飾エリア */
  artCard: {
    width: 700,
    height: 300,
    marginTop: 34,
    borderRadius: 40,
    borderWidth: 4,
  },
  badge: {
    size: 100,
    fontSize: 40,
  },
  cta: {
    marginTop: 40,
    width: 720,
    fontSize: 35,
    paddingY: 30,
    paddingX: 36,
    borderRadius: 28,
    boxShadow: '0 16px 32px rgba(16,51,73,0.16)',
    text: 'プロフィールのリンクから推しサーチへ',
  },
  decorations: {
    circles: [
      { size: 360, left: -130, bottom: -130, opacity: 0.26 },
      { size: 380, right: -140, bottom: -110, opacity: 0.2 },
      { size: 180, left: -60, top: 20, opacity: 0.16 },
    ] as DecorCircleConfig[],
    dots: [] as DecorDotConfig[],
  },
} as const;
