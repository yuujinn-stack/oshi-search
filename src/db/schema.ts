...
// ── VOD自動調査ジョブ（vod_investigation_jobs）──────────────────────────────────
// 調査対象CSVアップロード後の自動調査（AI Web検索）をジョブ単位で管理する。
// 個人情報は保存しない（person_nameは既存персons/worksテーブルにも存在する公開の
// 芸名・氏名で、既存機能と同じ扱い）。ジョブ・アイテムとも反映（apply）済み・完了後は
// 保持期間の目安なし運用中は保持し、管理者が不要と判断した時点で手動削除して構わない
// （物理削除のみ・論理削除フラグは持たない。反映結果自体はworks.vod_data /
// vod_recheck_logsに残るため、このテーブルは「作業中の一時データ」の位置づけ）。
export const vodInvestigationJobs = pgTable('vod_investigation_jobs', {
  id:          text('id').primaryKey(), // ランダムID（nanoid等）。他DDLと型を合わせるためtext
  status:      text('status').notNull().default('pending'), // pending|running|paused|completed
  createdBy:   text('created_by').notNull(), // 'admin:vod-recheck' 固定（既存のchangedBy運用と同じ）
  createdAt:   timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  updatedAt:   timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
}, (t) => [
  index('vij_status_idx').on(t.status),
  index('vij_created_at_idx').on(t.createdAt),
]);

// ── VOD自動調査ジョブの作品単位アイテム（vod_investigation_job_items）───────────
// 1行 = 1ジョブ内の1作品（canonical workId解決済み）。
// candidateProviders: AI調査結果（VodProvider[]相当、承認前のためDBのvod_dataには未反映）
// currentProvidersSnapshot: 調査実行時点の現在のVOD情報（確認画面の「現在」表示用に固定）
export const vodInvestigationJobItems = pgTable('vod_investigation_job_items', {
  id:                       serial('id').primaryKey(),
  jobId:                    text('job_id').notNull(),
  workId:                   text('work_id').notNull(), // resolveActiveWorkTargets解決済みのcanonical workId
  personName:               text('person_name').notNull(), // 代表人物（表示用）
  title:                    text('title').notNull(), // ジョブ作成時点のタイトルスナップショット
  workType:                 text('work_type').notNull(),
  releaseYear:              integer('release_year'),
  status:                   text('status').notNull().default('pending'), // pending|investigating|needs_review|approved|rejected|failed
  decision:                 text('decision').notNull().default('pending'), // pending|approved|rejected|needs_review|manual
  retryCount:               integer('retry_count').notNull().default(0),
  candidateProviders:       jsonb('candidate_providers').$type<unknown[]>(),
  currentProvidersSnapshot: jsonb('current_providers_snapshot').$type<unknown[]>(),
  manualProviders:          jsonb('manual_providers').$type<unknown[]>(), // 手動修正時の上書き内容
  errorMessage:             text('error_message'),
  ...
  index('viji_job_id_idx').on(t.jobId),
  index('viji_job_status_idx').on(t.jobId, t.status),
]);
