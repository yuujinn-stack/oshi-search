...
**目的：** `/person/[slug]` で書籍系とグッズ系が同一タブに混在していた問題を解消。
...
| 変更前 | 変更後 |
|---|---|
| `本・写真集`（sources: 写真集+本・雑誌） | `写真集・書籍`（sources: 写真集+本・雑誌+グッズ、titleKeywords: 書籍系） |
| `グッズ`（sources: グッズのみ） | `グッズ`（sources: グッズ+写真集+本・雑誌、残存品キャッチオール） |

**仕組み（crossSectionSeen）：**
1. セクションを順番に処理し、グローバルな `crossSectionSeen` set で商品を確保
2. `titleKeywords` があるセクション（写真集・書籍）→ キーワード一致品のみを確保
3. `titleKeywords` なしのセクション（グッズ）→ 先行セクションで確保されなかった残存品をすべて取得
4. グッズカテゴリに混入した書籍 → 写真集・書籍セクションが先にキーワードで確保
5. 写真集・本・雑誌カテゴリに混入したグッズ → 書籍キーワードに該当せずグッズセクションへ
